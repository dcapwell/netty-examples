...
import io.netty.channel.{ChannelHandlerContext, ChannelInboundHandlerAdapter, ChannelHandler}

object BlockClient extends Client {
  override def port: Int = 59903
  ...
  override def pipeline: List[ChannelHandler] = List(
    new ChannelInboundHandlerAdapter {
      override def channelActive(ctx: ChannelHandlerContext): Unit = {
        ...
        // send request header
        val reqHeader = RequestHeader(CurrentVersion, RequestType.PutBlock, blockId)
...
//        ctx.writeAndFlush(ctx.alloc().buffer(Longs.BYTES).writeLong(reqHeader.blockId.value))
        ctx.write(ctx.alloc().buffer(Longs.BYTES).writeLong(reqHeader.blockId.value))
